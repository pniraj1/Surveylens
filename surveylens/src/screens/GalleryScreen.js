...
import { getPhotoLog, deletePhotosFromLog } from '../utils/storageUtils';
...
export default function GalleryScreen({ navigation }) {
  ...
  const load = useCallback(async () => {
    const { getSettings } = await import('../utils/storageUtils');
    ...
    return (
      ...
      >
        ...
        <View style={styles.thumbMeta}>
          ...
          {item.gps && <Text style={styles.thumbGPS} numberOfLines={1}>📍 {item.gps.latitude}</Text>}
        </View>
      </TouchableOpacity>
    );
  };
